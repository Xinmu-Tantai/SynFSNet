...
from synfs.datasets.loveda_dataset import *
...
weights_name = f"convlsrnet-head88-bz{train_batch_size}-down2---test"


# Path configurations
weights_path = os.path.join(weights_dir, "loveda", weights_name)
log_name = os.path.join(logs_dir, "loveda", weights_name)
test_weights_name = "weights_name"
# Ensure directories exist
os.makedirs(os.path.join(weights_dir, "loveda"), exist_ok=True)
os.makedirs(os.path.join(logs_dir, "loveda"), exist_ok=True)
...
train_data_root = os.path.join(data_dir, "loveda", "Train")
test_data_root = os.path.join(data_dir, "loveda", "Test")

# define the dataloader
train_dataset = LoveDATrainDataset(transform=train_aug,data_root=train_data_root)
